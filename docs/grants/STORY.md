...
|---|---|---|
| **Celo** | **CIP-64.** Transaction type `0x7b`, EIP-1559 plus a `feeCurrency` field, so the node debits gas in USDC, USDT or cUSD. The user pays gas in the dollars they were already sent. | `celo/cip64-serializer.ts`, `celo/celo-fee-currency.ts` |
